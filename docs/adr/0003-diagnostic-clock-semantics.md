...
SCPN Fusion Fission Hybrid Core — ADR 0003
-->
...
bind. Two candidates apply to the configuration this repository owns:
`hybrid.source_blanket_response` (noncyclic feature) and
`model.synthetic_oscillator_coordinate` (numerical only). No cyclic or
event-relative route is catalogued for this family.

## Decision

1. The capability `diagnostic_clock_semantics` is implemented as a
   declaration model
   (`src/scpn_fusion_fission_hybrid_core/observability.py`): frozen
...
3. Alignment is enforced structurally: a channel must address an
   ...
   declared clock whose kind is compatible with the class (noncyclic
   to either non-simulation clock, numerical-only to the simulation
   clock).
4. The device-typical check is advisory only: sampling far above the
   second-scale delayed-neutron and thermal response of a subcritical
   blanket is flagged (H. A. Bethe, Phys. Today 32(5) (1979) 44, as
   the fusion-fission hybrid context; delayed-neutron timescales are
   standard reactor kinetics).
5. The reference plan mirrors canonical practice with synthetic
   ...
   model-owned synthetic oscillator (simulation clock). Every
   applicable candidate must be planned or explicitly deferred with a
   reason; every channel is `synthetic=True` by hard invariant. No SPO
   semantic-profile ingress is declared: the profile registry's
   `ingress_state` for this device family stays `not_declared` until a
   real, versioned adapter exists. No criticality instrumentation,
   nuclear-safety monitoring, or reactivity measurement capability is
   claimed or implied.
...
- No diagnostic, measurement, facility, adapter, or ingress is claimed
  ...
  none. Independent nuclear-safety authority is unaffected.
