...
# SCPN Fusion Fission Hybrid Core — diagnostic and clock semantics
...
identities for the fusion-fission hybrid configuration this
repository owns,
...
from scpn_fusion_fission_hybrid_core.configuration import ConsistencyFinding
...
BLANKET_SAMPLE_CEILING_HZ: Final = 1.0e5

SHOT_DURATION_CEILING_S: Final = 1.0e04
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-FUSION-FISSION-HYBRID-CORE"
                )
...
APPLICABLE_CANDIDATES: Final = (
    CandidateProfile(
        ...
        phenomenon=("fusion source and delayed blanket neutronic or thermal response"),
...
class DiagnosticChannelPlan:
    ...
    timing_uncertainty_s
        ...
        candidate is applicable to SCPN-FUSION-FISSION-HYBRID-CORE.
    acquisition_start_s
    ...
    def __post_init__(self) -> None:
        ...
        if candidate is None:
            raise DiagnosticPlanError(
                ...
                "to SCPN-FUSION-FISSION-HYBRID-CORE"
            )
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-FUSION-FISSION-HYBRID-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented hybrid-blanket diagnostic practice;
        ...
        for channel in self.channels:
            if (
                channel.candidate_id == "hybrid.source_blanket_response"
                and channel.sample_rate_hz > BLANKET_SAMPLE_CEILING_HZ
            ):
                findings.append(
                    ConsistencyFinding(
                        field=f"channels[{channel.identifier}].sample_rate_hz",
                        message=(
                            f"sampling at {channel.sample_rate_hz:.1f} Hz is "
                            "far above the second-scale delayed-neutron and "
                            "thermal response of a subcritical blanket "
                            "(Bethe 1979)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the source-following campaign scale of "
                            "up to ~10000 s (Bethe 1979 context)"
                        ),
                    )
                )
