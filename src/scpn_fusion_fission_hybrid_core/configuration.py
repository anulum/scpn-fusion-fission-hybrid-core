...
# SCPN Fusion Fission Hybrid Core — device configuration container

"""Device configuration container bound to the SPO reactor registry.

A :class:`DeviceConfiguration` composes a validated subcritical blanket
and neutron source under the single registry identifier this repository
owns. Strict subcriticality is a hard invariant; a multiplication factor
inside the documented criticality margin is flagged. Serialisation is
...
Phase Orchestrator code. Nothing here is a nuclear-safety,
...
OWNED_CONFIGURATIONS: Final = ("fusion_fission_hybrid",)
CRITICALITY_MARGIN_KEFF: Final = 0.98
...
class DeviceConfiguration:
    """Validated fusion-fission-hybrid device configuration.
    ...
    blanket
        Validated subcritical blanket.
    source
        Validated fusion neutron source.
    registry
    ...
    def __post_init__(self) -> None:
        ...
        if self.identifier not in OWNED_CONFIGURATIONS:
            raise DeviceConfigurationError(
                ...
                f"SCPN-FUSION-FISSION-HYBRID-CORE; owned: "
                f"{OWNED_CONFIGURATIONS!r}"
            )
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            ...
            the multiplication factor keeps the documented margin to
            criticality. Findings are advisory instruments, not machine
            or safety claims.
        """
        findings: list[ConsistencyFinding] = []
        keff = self.blanket.k_effective
        if keff > CRITICALITY_MARGIN_KEFF:
            findings.append(
                ConsistencyFinding(
                    field="blanket.k_effective",
                    message=(
                        f"k_effective {keff:.4f} is inside the documented "
                        f"margin to criticality (> "
                        f"{CRITICALITY_MARGIN_KEFF:.2f})"
                    ),
                )
            )
    ...
    def to_record(self) -> dict[str, Any]:
        ...
        return {
            ...
            "blanket": {
                "k_effective": self.blanket.k_effective,
                "fertile_class": self.blanket.fertile_class,
            },
            "source": {
                "source_rate_per_s": self.source.source_rate_per_s,
            },
            "registry": {
...
def configuration_from_record(record: Any) -> DeviceConfiguration:
    ...
    known = {"identifier", "blanket", "source", "registry"}
    unknown = sorted(set(record) - known)
    if unknown:
        raise DeviceConfigurationError(f"record: unknown fields {unknown!r}")
    blanket = _require_mapping(record, "blanket")
    source = _require_mapping(record, "source")
    registry = _require_mapping(record, "registry")
    return DeviceConfiguration(
        identifier=_string(record, "identifier"),
        blanket=SubcriticalBlanket(
            k_effective=_number(blanket, "k_effective"),
            fertile_class=_string(blanket, "fertile_class"),
        ),
        source=NeutronSource(
            source_rate_per_s=_number(source, "source_rate_per_s"),
        ),
        registry=RegistryBinding(
