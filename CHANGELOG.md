...
SCPN Fusion Fission Hybrid Core — CHANGELOG
-->
...
- Generated repository header artwork: `docs/assets/generate_header.py`
  ...
  domain surface (the driven subcritical blanket used by the README, the
  strictly open k_eff interval, and the declared neutron economy); every
  image carries the repository's nuclear non-claim in its text panel.
...
- Diagnostic and clock semantics model
  (`src/scpn_fusion_fission_hybrid_core/observability.py`), the second
  implemented capability at `computational_prototype`: frozen clock,
  channel, deferral, and plan objects aligned fail-closed with the
  pinned SPO observability-profile catalogue (candidate applicability,
  carrier admissibility, exact class-fixed evidence vocabularies,
  clock-kind compatibility); a cited advisory blanket-sampling check;
  canonical serialisation with SHA-256 digests and strict NaN-rejecting
  round-trip parsing (design record
  `docs/adr/0003-diagnostic-clock-semantics.md`).

- Device configuration model (`src/scpn_fusion_fission_hybrid_core/`), the first implemented
...
- Machine-readable domain manifest `reactor-domain.json` binding the project
  ...
  (configuration `fusion_fission_hybrid`).
